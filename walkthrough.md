# Walkthrough — Sarvam AI Multilingual Integration

## What Was Done

Integrated Sarvam AI as an alternative LLM backend into the existing NVIDIA RAG pipeline, adding multilingual Indian language support with automatic language detection and failsafe fallback.
...
|------|--------|--------------|
| [sarvam_rag.py](file:///d:/LLM%20RAG%20MODEL/sarvam_rag.py) | **NEW** | 310 lines — Sarvam LLM wrapper with 10 bugs fixed |
| [rag_agent.py](file:///d:/LLM%20RAG%20MODEL/rag_agent.py) | **MODIFIED** | ~120 lines added/changed |
| [.env.example](file:///d:/LLM%20RAG%20MODEL/.env.example) | **NEW** | API key template |

---

## Key Changes in rag_agent.py

### New CLI Flags
```
--llm {nvidia,sarvam,auto}     LLM backend for generation (default: nvidia)
--sarvam-model {sarvam-30b,sarvam-105b}  Sarvam model choice (default: sarvam-30b)
```

### Architecture Refactor

The monolithic `ask_agent()` was split into three clean functions:

```mermaid
graph LR
    A["ask_agent()"] --> B["_retrieve_chunks()"]
    A --> C{"llm_backend?"}
    C -->|nvidia| D["_generate_nvidia()"]
    C -->|sarvam| E["_generate_sarvam()"]
    C -->|auto| F["detect_is_indic()"]
    F -->|Indic script| E
    F -->|Latin script| D
    E -->|"on failure"| D
```
...
if not NVIDIA_API_KEY:
    raise EnvironmentError(
        ...
        "Run:  export NVIDIA_API_KEY='nvapi-...'"
    )
...
def ask_agent(
    ...
    # Retrieve more candidates when reranking is enabled
    ...
    # Rerank candidates to get the most relevant ones
    if use_rerank and len(candidate_chunks) > 0:
        print(f"Reranking {len(candidate_chunks)} candidates...")
        final_chunks = rerank_chunks(question, candidate_chunks, top_n=rerank_top_n)
    else:
...
Added `detect_is_indic()` — a zero-dependency Unicode script detector that checks if ≥15% of alphabetic characters belong to Indian scripts (Devanagari, Bengali, Tamil, Telugu, Kannada, Malayalam, Gujarati, Gurmukhi, Oriya).

### Fallback Behavior

When `--llm sarvam` or `--llm auto` routes to Sarvam, any failure automatically retries with NVIDIA:
```
Sarvam failed (ConnectionError), falling back to NVIDIA...
Thinking (NVIDIA)...
```

---

## 10 Bugs Fixed in Original sarvam_rag.py

| # | Severity | Bug | Fix |
|---|----------|-----|-----|
| 1 | 🔴 Critical | `max_tokens` → wrong API param | Changed to `max_completion_tokens` |
| 2 | 🟡 Medium | Redundant `api-subscription-key` header | Removed (Chat API uses Bearer only) |
| 3 | 🔴 Critical | No `.env` file exists | Created `.env.example` template |
| 4 | 🟡 Medium | No response parsing safety | Added try/except with clear error messages |
| 5 | 🟢 Low | SSE parser too strict (`data: ` only) | Handles both `data:` and `data: ` |
| 6 | 🟡 Medium | No connection reuse | Added `requests.Session()` pooling |
| 7 | 🟢 Low | `reasoning_effort` not validated | Added validation in `__init__` |
| 8 | 🟡 Medium | Retry catches bare `Exception` | Separate handlers per exception type |
| 9 | 🟢 Low | Stream response never closed | Added `try/finally: resp.close()` |
| 10 | 🟢 Cosmetic | Language selector UI unused | Documented (auto-detect handles this) |
...
| Test | Result |
|------|--------|
| `sarvam_rag.py` syntax | ✅ Pass |
| `rag_agent.py` syntax | ✅ Pass |
| Cross-module imports (no circular deps) | ✅ Pass |
| Language detection: Hindi `ताज महल कब बना` | ✅ `True` |
| Language detection: English `Where is the Taj Mahal?` | ✅ `False` |
| Language detection: Bengali `টাটা স্টীল` | ✅ `True` |
| Language detection: Mixed `मुझे बताओ about this` | ✅ `True` |
| `ask_agent()` signature has `llm_backend` + `sarvam_model` | ✅ Verified |
...
### 1. Set up API key
```bash
# Copy the template and add your Sarvam key
copy .env.example .env
# Edit .env and add: SARVAM_API_KEY=your-key-here
```

### 2. Run with Sarvam
```bash
# Force Sarvam for all queries
python rag_agent.py --llm sarvam --query "ताज महल कहाँ है?"

# Auto-detect language (Hindi→Sarvam, English→NVIDIA)
python rag_agent.py --llm auto

# Use the flagship 105B model
python rag_agent.py --llm sarvam --sarvam-model sarvam-105b

# Default (unchanged) — NVIDIA only
python rag_agent.py
```
...
answer = rag.query("इस document में क्या है?", chunks)
```
